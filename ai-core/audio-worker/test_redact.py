...
from speaker_id import cosine_similarity, match_speaker  # noqa: E402
...
def test_cosine_similarity_and_match_speaker():
    vec_a = [1.0, 0.0, 0.0]
    vec_b = [1.0, 0.0, 0.0]
    vec_c = [0.0, 1.0, 0.0]
    ...
    assert cosine_similarity([], vec_a) == 0.0

    registered = {"user-1": vec_a, "user-2": vec_c}
    muid, score = match_speaker(vec_a, registered, threshold=0.8)
    assert muid == "user-1"
    assert score == 1.0

    muid, score = match_speaker(vec_a, {"user-2": vec_c}, threshold=0.8)
    assert muid is None
    assert score == 0.0
